...
function GenerateQuad(left,right,bot,top,edge){
	...
	return(
	[
		left ,  bot,  (perlin.get(left ,bot)*lem+lem)*.5,
		right,  bot,  (perlin.get(right,bot)*rem+rem)*.5,
		right,  top,  (perlin.get(right,top)*rem+rem)*.5,

		right,  top,  (perlin.get(right,top)*rem+rem)*.5,
		left ,  top,  (perlin.get(left ,top)*lem+lem)*.5,
		left ,  bot,  (perlin.get(left ,bot)*lem+lem)*.5
	])
}
